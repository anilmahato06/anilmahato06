<div align="center">

# 👋 Hi, I'm Anil Mahato

### 🎬 Video Editor • 🎨 Graphic Designer • 💻 Computer Operator • 🎓 B.Sc. CSIT Student

I combine **creative media production** with growing **computer science and technical skills**, with a focus on visual storytelling, design, digital workflows, and continuous learning.

<img src="https://readme-typing-svg.demolab.com?font=Inter&weight=600&size=22&duration=3000&pause=900&color=58A6FF&center=true&vCenter=true&width=700&lines=Anil+Mahato;Video+Editor+%26+Graphic+Designer;B.Sc.+CSIT+Student;Creative+Media+%2B+Technology;Learning.+Creating.+Building." alt="Animated introduction showing Anil Mahato's roles and interests" />

<br>

<a href="YOUR_PORTFOLIO_URL">
  <img src="https://img.shields.io/badge/Portfolio-YOUR_PORTFOLIO_URL-111827?style=for-the-badge&logo=googlechrome&logoColor=white" alt="Portfolio placeholder" />
</a>
<a href="https://github.com/YOUR_GITHUB_USERNAME">
  <img src="https://img.shields.io/badge/GitHub-YOUR_GITHUB_USERNAME-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub profile placeholder" />
</a>
<a href="YOUR_LINKEDIN_URL">
  <img src="https://img.shields.io/badge/LinkedIn-YOUR_LINKEDIN_URL-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn placeholder" />
</a>
<a href="mailto:YOUR_EMAIL">
  <img src="https://img.shields.io/badge/Email-YOUR_EMAIL-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Email placeholder" />
</a>

<br><br>

<img src="https://img.shields.io/badge/📍%20Dhanusha%2C%20Nepal-111827?style=flat-square" alt="Based in Dhanusha, Nepal" />
<img src="https://img.shields.io/badge/🤝%20Open%20to%20Collaboration-111827?style=flat-square" alt="Open to collaboration" />
<img src="https://img.shields.io/badge/🎓%20B.Sc.%20CSIT%20Student-111827?style=flat-square" alt="B.Sc. CSIT student" />

</div>

---

## 👤 About Me

I'm **Anil Mahato**, a creative and technology-focused professional from **Dhanusha, Nepal**. My work sits at the intersection of **visual storytelling, graphic design, computer operations, and computer science education**.

I am developing practical skills across both creative media and technology while continuing my studies in **Bachelor of Science in Computer Science and Information Technology**.

### Quick Facts

| | |
|---|---|
| 🎬 **Currently working** | Video Editor & Graphic Designer at Mithila Series |
| 🎓 **Studying** | B.Sc. in Computer Science and Information Technology |
| 💻 **Technical foundation** | C, C++, HTML, Microsoft Office, Data Entry, Digital Documentation |
| 🌱 **Currently learning** | Data Structures, Web Development, Git & GitHub, professional creative workflows |
| 📷 **Interested in** | Photography, Videography, Visual Storytelling, Content Creation |
| 🤝 **Open to** | Creative Collaborations, Freelance Projects, Internships, Entry-Level IT Opportunities |
| 📍 **Based in** | Dhanusha, Nepal |

---

## 🧑‍💻 Developer / Creator Snapshot

```javascript
const anilMahato = {
  name: "Anil Mahato",
  location: "Dhanusha, Nepal",

  roles: [
    "Video Editor",
    "Graphic Designer",
    "Computer Operator",
    "B.Sc. CSIT Student"
  ],

  education: {
    current: "B.Sc. in Computer Science and Information Technology",
    institution: "Rajarshi Janak University"
  },

  creativeFocus: [
    "Video Editing",
    "Graphic Design",
    "Photography",
    "Videography"
  ],

  technicalFocus: [
    "C",
    "C++",
    "HTML",
    "Microsoft Office",
    "Data Entry",
    "Digital Documentation",
    "Computer Operations"
  ],

  currentlyLearning: [
    "Data Structures & Programming Fundamentals",
    "HTML & Web Development",
    "Professional Video Editing Workflows",
    "Graphic Design & Brand Consistency",
    "Git & GitHub",
    "Digital Records & Office Workflows"
  ],

  openFor: [
    "Creative Collaboration",
    "Freelance Projects",
    "Internships",
    "Entry-Level IT Opportunities"
  ]
};
```

---

## 🎯 What I Do

<table>
<tr>
<td width="50%" valign="top">

### 🎬 Video Editing

Creating and refining visual content with a focus on clear storytelling, pacing, and presentation.

</td>
<td width="50%" valign="top">

### 🎨 Graphic Design

Developing visual compositions with attention to design, consistency, and communication.

</td>
</tr>
<tr>
<td width="50%" valign="top">

### 🖥️ Computer Operations

Working with digital records, documentation, data entry, and day-to-day computer-based workflows.

</td>
<td width="50%" valign="top">

### 💻 Programming & Web Fundamentals

Building foundational knowledge through C, C++, HTML, data structures, and web development basics.

</td>
</tr>
</table>

---

## 🛠️ Skills & Tools

### 🎨 Creative

<img src="https://img.shields.io/badge/Video%20Editing-111827?style=for-the-badge&logoColor=white" alt="Video Editing" />
<img src="https://img.shields.io/badge/Graphic%20Design-111827?style=for-the-badge&logoColor=white" alt="Graphic Design" />
<img src="https://img.shields.io/badge/Photography-111827?style=for-the-badge&logoColor=white" alt="Photography" />
<img src="https://img.shields.io/badge/Videography-111827?style=for-the-badge&logoColor=white" alt="Videography" />

### 💻 Technical

<img src="https://img.shields.io/badge/C-00599C?style=for-the-badge&logo=c&logoColor=white" alt="C" />
<img src="https://img.shields.io/badge/C%2B%2B-00599C?style=for-the-badge&logo=cplusplus&logoColor=white" alt="C++" />
<img src="https://img.shields.io/badge/HTML-E34F26?style=for-the-badge&logo=html5&logoColor=white" alt="HTML" />
<img src="https://img.shields.io/badge/Microsoft%20Office-D83B01?style=for-the-badge&logo=microsoftoffice&logoColor=white" alt="Microsoft Office" />
<img src="https://img.shields.io/badge/Data%20Entry-111827?style=for-the-badge&logoColor=white" alt="Data Entry" />
<img src="https://img.shields.io/badge/Digital%20Documentation-111827?style=for-the-badge&logoColor=white" alt="Digital Documentation" />
<img src="https://img.shields.io/badge/Computer%20Operations-111827?style=for-the-badge&logoColor=white" alt="Computer Operations" />

---

## 💼 Experience

### 🎬 Video Editor & Graphic Designer
**Mithila Series**  
`2025 → Present`

Working in a creative role focused on **video editing and graphic design**.

### 🖥️ Computer Operator
**I.P.S Education Hub**  
`2024 → 2025`

Worked in a computer operations role supporting **digital and office-based workflows**.

---

## 🎓 Education

### 🎓 Bachelor of Science in Computer Science and Information Technology

**Rajarshi Janak University**  
`2024 – Present`

### 💻 Diploma in Computer Engineering (TVET)

**Shiva Shakti Higher Secondary School**  
`2019 – 2023`

---

## 🖼️ Featured Work

> **Portfolio links are placeholders until real work URLs are provided.**

<table>
<tr>
<td width="33%" valign="top">

### 🎬 Video Editing Portfolio

A dedicated space for selected video editing work.

**Link:**  
`YOUR_VIDEO_PORTFOLIO_URL`

</td>
<td width="33%" valign="top">

### 🎨 Graphic Design Portfolio

A dedicated space for selected graphic design work.

**Link:**  
`YOUR_GRAPHIC_DESIGN_PORTFOLIO_URL`

</td>
<td width="33%" valign="top">

### 💻 Programming Practice

A place to showcase programming practice and repositories.

**Repositories:**  
https://github.com/YOUR_GITHUB_USERNAME?tab=repositories

</td>
</tr>
</table>

> When real projects are added, each project should include the **title, short description, technologies, Anil's contribution, live demo, and source code** where applicable.

---

## 🌱 Currently Learning

```text
Data Structures & Programming Fundamentals
            ↓
HTML & Web Development
            ↓
Professional Video Editing Workflows
            ↓
Graphic Design & Brand Consistency
            ↓
Git & GitHub
            ↓
Digital Records & Office Workflows
```

No artificial percentages. Progress is represented as a learning journey rather than fabricated skill scores.

---

## 📊 GitHub Analytics

> Replace `YOUR_GITHUB_USERNAME` with the actual GitHub username before publishing.

<div align="center">

<a href="https://github.com/YOUR_GITHUB_USERNAME">
  <img
    src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=YOUR_GITHUB_USERNAME&theme=tokyonight"
    alt="GitHub profile summary for Anil Mahato"
    width="90%"
  />
</a>

<br><br>

<a href="https://github.com/YOUR_GITHUB_USERNAME">
  <img
    src="https://github-readme-stats.vercel.app/api?username=YOUR_GITHUB_USERNAME&show_icons=true&hide_border=true&theme=tokyonight&rank_icon=github"
    alt="GitHub statistics for Anil Mahato"
    width="48%"
  />
</a>
<a href="https://github.com/YOUR_GITHUB_USERNAME">
  <img
    src="https://streak-stats.demolab.com?user=YOUR_GITHUB_USERNAME&theme=tokyonight&hide_border=true"
    alt="GitHub contribution streak for Anil Mahato"
    width="48%"
  />
</a>

</div>

---

## ❤️ Personal Interests

<table>
<tr>
<td align="center">📷<br><strong>Photography</strong></td>
<td align="center">🎥<br><strong>Videography</strong></td>
<td align="center">🎨<br><strong>Visual Design</strong></td>
<td align="center">🎬<br><strong>Storytelling</strong></td>
<td align="center">💻<br><strong>Creative Technology</strong></td>
<td align="center">🌱<br><strong>Continuous Learning</strong></td>
</tr>
</table>

---

## 🌐 Languages

<img src="https://img.shields.io/badge/Maithili-111827?style=for-the-badge" alt="Maithili" />
<img src="https://img.shields.io/badge/Nepali-111827?style=for-the-badge" alt="Nepali" />
<img src="https://img.shields.io/badge/Hindi-111827?style=for-the-badge" alt="Hindi" />
<img src="https://img.shields.io/badge/English-111827?style=for-the-badge" alt="English" />

---

## ❓ FAQ

<details>
<summary><strong>What does Anil Mahato do?</strong></summary>

Anil Mahato is a **Video Editor, Graphic Designer, Computer Operator, and B.Sc. CSIT Student** based in Dhanusha, Nepal.
</details>

<details>
<summary><strong>What creative skills does Anil have?</strong></summary>

His creative skills include **Video Editing, Graphic Design, Photography, and Videography**.
</details>

<details>
<summary><strong>What technical skills is Anil learning?</strong></summary>

His technical foundation includes **C, C++, HTML, Microsoft Office, Data Entry, Digital Documentation, and Computer Operations**. He is also currently learning **Data Structures, Web Development, Git & GitHub, and digital workflows**.
</details>

<details>
<summary><strong>Is Anil available for collaboration?</strong></summary>

Yes. Anil is open to **Creative Collaborations, Freelance Projects, Internships, and Entry-Level IT Opportunities**.
</details>

<details>
<summary><strong>What is Anil studying?</strong></summary>

He is studying **Bachelor of Science in Computer Science and Information Technology** at **Rajarshi Janak University**.
</details>

---

## 🤝 Let's Connect

I'm interested in connecting with **creators, developers, organizations, and teams** working across creative media and technology.

<div align="center">

<a href="mailto:YOUR_EMAIL">
  <img src="https://img.shields.io/badge/📧%20Email-YOUR_EMAIL-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Email placeholder" />
</a>
<a href="YOUR_LINKEDIN_URL">
  <img src="https://img.shields.io/badge/💼%20LinkedIn-YOUR_LINKEDIN_URL-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn placeholder" />
</a>
<a href="YOUR_PORTFOLIO_URL">
  <img src="https://img.shields.io/badge/🎬%20Portfolio-YOUR_PORTFOLIO_URL-6E56CF?style=for-the-badge&logo=googlechrome&logoColor=white" alt="Portfolio placeholder" />
</a>
<a href="https://github.com/YOUR_GITHUB_USERNAME">
  <img src="https://img.shields.io/badge/🐙%20GitHub-YOUR_GITHUB_USERNAME-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub profile placeholder" />
</a>

</div>

---

<div align="center">

### Thanks for visiting my profile! 👋

<img
  src="https://komarev.com/ghpvc/?username=YOUR_GITHUB_USERNAME&style=flat-square&label=Profile%20Views"
  alt="GitHub profile views for Anil Mahato"
/>

</div>
